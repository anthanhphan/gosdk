...
// ContextInterface defines the interface that FiberContext implements.
// This avoids import cycles by not importing the main aurelion package.
...
// Cookie represents an HTTP cookie (duplicated here to avoid import cycle).
type Cookie struct {
	Name     string
	Value    string
	Path     string
...
const trackedLocalsKey = TrackedLocalsKey

// FiberContext wraps fiber.Ctx to implement aurelion.Context.
...
func NewFiberContext(fiberCtx *fiber.Ctx) ContextInterface {
	if fiberCtx == nil {
		return nil
	}
...
func FiberFromContext(ctx ContextInterface) (*fiber.Ctx, bool) {
	if ctx == nil {
		return nil, false
	}
...
// HandlerToFiber converts a Handler function to fiber.Handler.
func HandlerToFiber(handler func(ContextInterface) error) fiber.Handler {
	if handler == nil {
...
// MiddlewareToFiber converts a Middleware function to fiber.Handler.
func MiddlewareToFiber(middleware func(ContextInterface) error) fiber.Handler {
	if middleware == nil {
...
func (c *FiberContext) Status(status int) ContextInterface {
...
func (c *FiberContext) RequestID() string {
	if id := c.Locals(ContextKeyRequestID); id != nil {
