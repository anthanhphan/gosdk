	...
	}{
		{
			...
			check: func(t *testing.T, builder *RouteBuilder) {
				if builder.route.Methods[0] != core.GET {
					t.Errorf("Method() = %v, want GET", builder.route.Methods[0])
				}
			},
		},
		{
			...
			method: core.POST,
			check: func(t *testing.T, builder *RouteBuilder) {
				if builder.route.Methods[0] != core.POST {
					t.Errorf("Method() = %v, want POST", builder.route.Methods[0])
				}
			},
		},
	}
...
func TestRouteBuilder_GET(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, builder *RouteBuilder) {
				if builder.route.Methods[0] != core.GET {
					t.Errorf("GET() = %v, want GET", builder.route.Methods[0])
				}
			},
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, route *Route) {
				...
				if route.Methods[0] != core.GET {
					t.Errorf("Build() method = %v, want GET", route.Methods[0])
				}
