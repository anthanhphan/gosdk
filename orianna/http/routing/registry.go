...
func (rr *RouteRegistry) RegisterRoutes(routes ...Route) error {
	...
	// Phase 1: Validate all routes and check for duplicates without committing
	...
	for i := range routes {
		...
		for _, m := range methods {
			key := routeKey(m, "", routes[i].Path)
			if _, exists := rr.registeredPaths[key]; exists {
				return fmt.Errorf("route at index %d: %w: %s", i, core.ErrDuplicateRoute, key)
			}
			if _, exists := tempPaths[key]; exists {
				return fmt.Errorf("route at index %d: %w: %s (duplicate within batch)", i, core.ErrDuplicateRoute, key)
			}
			tempPaths[key] = struct{}{}
		}
	}

	// Phase 2: Commit all routes (no errors possible past this point)
	for i := range routes {
		rr.applyProtectionMiddleware(&routes[i])
		rr.routes = append(rr.routes, routes[i])
	}
	for key := range tempPaths {
		rr.registeredPaths[key] = struct{}{}
	}
	return nil
}
...
func routeKey(method core.Method, prefix, path string) string {
	...
	b.Grow(len(methodStr) + 1 + len(prefix) + len(path))
	b.WriteString(methodStr)
	b.WriteByte(' ')
