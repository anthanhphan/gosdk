...
func TestHTTPChecker_Check_ServerError(t *testing.T) {
	...
	if check.Status != health.StatusUnhealthy {
		t.Errorf("Status = %v, want %v (500 should be unhealthy)", check.Status, health.StatusUnhealthy)
	}
}
...
func TestHTTPChecker_Check_ClientError(t *testing.T) {
	...
	if check.Status != health.StatusDegraded {
		t.Errorf("Status = %v, want %v (404 should be degraded)", check.Status, health.StatusDegraded)
	}
}
...
func TestHTTPChecker_Check_ConnectionRefused(t *testing.T) {
	...
	if check.Status != health.StatusUnhealthy {
		t.Errorf("Status = %v, want %v", check.Status, health.StatusUnhealthy)
	}
...
func TestHealthManager_Multiple_Checkers(t *testing.T) {
	...
	for i := 0; i < 15; i++ {
		...
		mgr.Register(health.NewCustomChecker(name, func(ctx context.Context) health.HealthCheck {
...
func TestHealthManager_DegradedDoesNotOverrideUnhealthy(t *testing.T) {
	...
	mgr.Register(health.NewCustomChecker("unhealthy", func(ctx context.Context) health.HealthCheck {
		return health.HealthCheck{Status: health.StatusUnhealthy}
	}))
	mgr.Register(health.NewCustomChecker("degraded", func(ctx context.Context) health.HealthCheck {
		return health.HealthCheck{Status: health.StatusDegraded}
	}))
	mgr.Register(health.NewCustomChecker("healthy", func(ctx context.Context) health.HealthCheck {
		return health.HealthCheck{Status: health.StatusHealthy}
	}))
	...
	if report.Status != health.StatusUnhealthy {
		t.Errorf("Status = %v, want %v (unhealthy takes precedence)", report.Status, health.StatusUnhealthy)
	}
}
...
func TestCustomChecker_PreservesName(t *testing.T) {
	checker := health.NewCustomChecker("fallback", func(ctx context.Context) health.HealthCheck {
		return health.HealthCheck{
			Name:    "explicit-name",
			Status:  health.StatusHealthy,
			Message: "OK",
...
func TestWithHealthChecker_CreatesManager(t *testing.T) {
	...
	checker := health.NewCustomChecker("test-checker", func(ctx context.Context) health.HealthCheck {
		return health.HealthCheck{Status: health.StatusHealthy}
	})
